...
dy_file = "/afs/cern.ch/work/m/mrieger/public/hbt/external_files/custom_dy_files/hbt_corrections.json.gz"
...
def plot_function(var_instance: od.Variable, file_version: str, dy_weights=None, filter_events: bool = True, channel_id=None, b_tag_cut=None, n_jet_cut=None):  # noqa: E501
    ...
    plt.savefig(f"plot_{file_version}.png")
...
def plot_2d_function(var_instance: od.Variable, file_version: str, dy_weights=None, channel_id=None):  # noqa: E501
    ...
    plt.xlabel(f"{var_instance.x_title} ({var_instance.unit})" if var_instance.unit else var_instance.x_title)
    plt.ylabel("DY weights")
    plt.title(f"{file_version} weight correlation: {var_name}")
    plt.grid(True, linestyle='--', alpha=0.5)

    plt.savefig(f"plot_{file_version}_{var_name}_correlation.pdf")
...
model.load_state_dict(torch.load("dy_regression_weights.pth"))
model.eval()
...
dy_ll_pt = variable_list["ll_pt"][1]
dy_n_jet = variable_list["n_jet"][1]
dy_n_btag_pnet = variable_list["n_btag_pnet"][1]
dy_bb_pt = variable_list["bb_pt"][1]
dy_bb_mass = variable_list["bb_mass"][1]
...
final_weights = final_weights * pred_correction.detach().cpu().numpy().flatten()
...
np.save("dy_event_weights_regression.npy", final_weights)
print(f"Event weights from regression model saved. Shape: {final_weights.shape}")
np.save("dy_event_weights_correctionlib.npy", correctionlib_weight)
print(f"Event weights from correctionlib saved. Shape: {correctionlib_weight.shape}")
