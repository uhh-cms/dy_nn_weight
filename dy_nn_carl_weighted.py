...
dy_weights = dy_weights / (torch.mean(dy_weights) + 1e-8)
...
data_weights = torch.ones(len(data_train_inputs), 1)  # weight=1 for data events
...
train_weights = torch.cat((data_weights, dy_weights[dy_train_idx]), dim=0)  # weight=1 for data, dy weights for dy events
...
torch.save(scaling_params, "input_scaling_params_v2.pth")
print("Saved input scaling parameters to input_scaling_params_v2.pth")
